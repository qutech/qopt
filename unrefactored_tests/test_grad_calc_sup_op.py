...
h_drift = [qutip.control_2.matrix.ControlDense(np.zeros((2, 2), dtype=complex))
           for _ in range(num_u)]
h_noise = [.5 * 2 * math.pi * qutip.control_2.matrix.ControlDense(
    qutip.sigmaz())]
...
initial_state = qutip.control_2.matrix.ControlDense(np.eye(4, dtype=complex))

spectral_density = 100e3

"""
def spec_density_white(f):
    return 100e3 * np.ones_like(f)


qs_noise_parameters = qutip.control_2.noise.NTGColoredNoise(
    noise_spectral_density=spec_density_white,
    dt=dt,
    n_samples_per_trace=num_u,
    n_traces=1, n_noise_operators=1
)
"""
tau_u = [100e-9 / over_sample_rate for _ in range(num_u)]

t_slot_comp = qutip.control_2.tslotcomp.TSCompLindblad(
    h_drift=h_drift,
    h_ctrl=h_ctrl,
