...
class SchroedingerSMCControlNoise(SchroedingerSMonteCarlo):
    ...
    def __init__(
        ...
        def noise_amplitude_function(noise_samples: np.array,
            ...
            """
            noise_amplitudes = np.zeros((noise_samples.shape[0],noise_samples.shape[1],control_amplitudes.shape[1]), dtype=complex)
...
class SolverJAX(Solver):
    ...
    def __init__(
        ...
        if type(h_drift) in [matrix.DenseOperator, matrix.SparseOperator,
            ...
            self._h_drift_jnp = jnp.expand_dims(h_drift[0].data,0)
            self.h_drift = h_drift * self.transfer_function.num_x
        else:
            self._h_drift_jnp = jnp.array([h.data for h in h_drift])
            self.h_drift = h_drift
        
        self._h_ctrl_jnp = jnp.array([h.data for h in h_ctrl])
        self._transferred_time_jnp = jnp.array(self.transferred_time)
        if initial_state is None:
            dim = self.h_ctrl[0].shape[0]
            self.initial_state = matrix.DenseOperatorJAX(jnp.eye(dim))
        else:
            self.initial_state = matrix.DenseOperatorJAX(initial_state)
        self._initial_state_jnp = self.initial_state.data         
            
        self._prop_jnp = None
        self._reversed_prop_jnp = None
        self._fwd_prop_jnp = None
    ...
    def reset_cached_propagators(self):
        ...
        self._prop = None #perhaps nonexistent?
        self._fwd_prop = None
        self._derivative_prop = None
        self._reversed_prop = None
        self.pulse_sequence = None
    ...
    @property
    def forward_propagators_jnp(self) -> jnp.ndarray:

        if self._fwd_prop_jnp is None:
            self._compute_forward_propagation_jnp()
        return self._fwd_prop_jnp
    
    @property
    def reversed_propagators_jnp(self) -> jnp.ndarray:

        if self._reversed_prop_jnp is None:
            self._compute_reversed_propagation_jnp()
        return self._reversed_prop_jnp
    
    @property
    def frechet_deriv_propagators_jnp(self) -> jnp.ndarray:
        
        if self._derivative_prop_jnp is None:
            self._compute_propagation_derivatives_jnp()
        return self._derivative_prop_jnp
    ...
    def _compute_propagation(self) -> None:
        ...
        self._prop = [matrix.DenseOperatorJAX(p) for p in self._prop_jnp]
...
class SchroedingerSolverJAX(SolverJAX):
    ...
    def __init__(self,
        ...
        if self.exponential_method != "Frechet":
            print("Other than Frechet ignored")
    ...
    def _compute_propagation_jnp(
            ...
            -> None:
        """See base class. """
        super()._compute_propagation_jnp()
...
class SchroedingerSMonteCarloJAX(SchroedingerSolverJAX):
    ...
    def __init__(
        ...
        self.h_noise = h_noise
        self._h_noise_jnp = jnp.array([h.data for h in h_noise])
        self.noise_trace_generator = noise_trace_generator
        self.noise_amplitude_function = noise_amplitude_function
        self.processes = processes
    ...
    def _compute_dyn_gen_noise(self) -> jnp.ndarray:
        """
        Computes the dynamics generators for the perturbed and unperturbed
        Schroedinger equation.
        ...
        noise_samples = jnp.array(self.noise_trace_generator.noise_samples)
        # we transpose, so we iterate over the time last
        noise_samples = jnp.transpose(noise_samples, (2, 1, 0))
    ...
    def _compute_propagation_derivatives_jnp(self) -> None:
        ...
        if not self.frechet_deriv_approx_method:
            ...
            super()._compute_propagation_derivatives_jnp()
...
class LindbladSolverJAX(SchroedingerSolverJAX):
    ...
    def __init__(
        ...
        self._diss_sup_op_jnp = None
        self._diss_sup_op_deriv_jnp = None
    ...
    def _calc_diss_sup_op_jnp(self) -> jnp.ndarray:
        r"""
        Calculates the dissipative super operator as described in the class
        doc string.
    ...
    def _compute_dyn_gen(self) -> jnp.ndarray:
        ...
        if self._diss_sup_op_jnp is None:
            self._diss_sup_op_jnp = self._calc_diss_sup_op_jnp()

        identity_operator = jnp.identity(self._dyn_gen[0].shape[0])
        sup_op_dyn_gen = []
    ...
    def _compute_propagation_jnp(
            ...
            -> None:
        """See base class. """
        super(SchroedingerSolverJAX,self)._compute_propagation_jnp()
...
class LindbladSControlNoiseJAX(LindbladSolverJAX):
    ...
    def _compute_propagation(self):
        ...
        if not self.constant_lindblad_operators or \
                self.incoherent_dyn_gen is None:
            transfer_matrix = self.transfer_function.transfer_matrix
            self.incoherent_dyn_gen = jnp.einsum('ijk,klm,k->ilm',
                                                transfer_matrix,
                                                self.lindblad_super_operator,
                                                self.noise_psd)
        dim = self._dyn_gen[0].shape[0]
